window.LaravelLabLayout = (() => {
  ...
  const renderNavbar = () => {
    ...
    target.innerHTML = `
      <nav class="navbar navbar-expand-lg app-navbar sticky-top" aria-label="Navigasi utama">
        <div class="container">
          <a class="navbar-brand" href="${rootPath}index.html" aria-label="Belajar Laravel">
            <img class="brand-logo brand-logo-light" src="${rootPath}assets/images/logo-light.svg" alt="Belajar Laravel" />
  ...
  const init = () => {
    document.body.classList.toggle("dark-mode", Boolean(window.LaravelLabProgress?.state.darkMode));
    renderNavbar();
    renderFooter();
    renderToast();
  };
