window.LaravelLabApp = (() => {
  const data = window.LaravelLabData;
  const progress = window.LaravelLabProgress;
  ...
  const debugAttempts = {};
  const LESSON_RECALL_STORAGE_KEY = "laravel-beginner-lab-lesson-recall-v1";
  const RECALL_STORAGE_KEY = "laravel-beginner-lab-recall-answers-v1";
  const DEBUG_ATTEMPT_STORAGE_KEY = "laravel-beginner-lab-debug-attempts-v1";
  let toastInstance;
  ...
  const renderLessonCodePreview = (item) => {
    const previewOutput = escapeHTML(item.previewOutput || "Jalankan contoh ini pada project Laravel lokal untuk melihat hasil aslinya.");
  ...
  <body>
    ...
    return `
      <div class="lesson-code-preview">
        <div class="lesson-preview-label"><i class="bi bi-eye"></i> Preview alur Laravel</div>
        <iframe
...
<html lang="id">
    ...
    if (example.type === "article") {
      content = `
        <div class="example-article">
          ...
          <footer>Ditulis untuk teman belajar Laravel</footer>
        </div>`;
    }
  ...
  const runLaravelEditorPreview = () => {
    window.LaravelLabEditor?.runEditor?.();
  };
  ...
  const initCssColorPicker = (cssInput, onChange) => {
    ...
    const presets = ["#FFFFFF", "#111827", "#FF2D20", "#D91F16", "#16A36A", "#0EA5B7", "#7C3AED", "#DC4C64", "#FBBF24", "#22C55E", "#38BDF8", "#64748B"];
  ...
  const initLaravelEditorEnhancements = () => {
    if (document.body.dataset.editorEnhancements === "laravel") return;
    const routesInput = getElement("routesEditor");
    const controllerInput = getElement("controllerEditor");
    const bladeInput = getElement("bladeEditor");
    const cssInput = getElement("cssEditor");
    if (!routesInput || !controllerInput || !bladeInput || !cssInput) return;

    document.body.dataset.editorEnhancements = "laravel";
    const inputs = [routesInput, controllerInput, bladeInput, cssInput];
    editorTagSuggest = initHtmlTagSuggest(bladeInput, runLaravelEditorPreview);
    editorColorPicker = initCssColorPicker(cssInput, runLaravelEditorPreview);
    editorUndoHistory = initEditorUndoHistory(inputs, runLaravelEditorPreview);
    initEditorTabBehavior(inputs, runLaravelEditorPreview);
  ...
  const init = () => {
    ...
    renderProjects();
    updateProgress();
    window.LaravelLabEditor?.init();
    initLaravelEditorEnhancements();
...
document.addEventListener("DOMContentLoaded", window.LaravelLabApp.init);
